...
export async function POST(request: Request) {
  try {
    ...
    const tables = [
      ...
      {
        name: "detected_competitors",
        sql: `
          CREATE TABLE "detected_competitors" (
              "id" TEXT NOT NULL,
              "analysisId" TEXT NOT NULL,
    ...
    // Check if each table exists, create if not
    for (const table of tables) {
      try {
        ...
        results.tablesChecked.push({ name: table.name, exists: tableExists });
    ...
    // Create foreign keys (safe - won't error if exists)
    try {
      ...
      results.errors.push({ step: "foreign_keys", error: error.message });
    }
